...
public abstract class SingleSceneSingleton<T> : MonoBehaviour where T : SingleSceneSingleton<T>
{
    ...
    private static readonly string SINGLETON_NAME = $"(singleton) {typeof(T)}";
    ...
    private static T Init()
    {
        ...
        lock (lockObject)
        {
            ...
            if (instance == null)
            {
                GameObject singleton = new() { name = SINGLETON_NAME };

                instance = singleton.AddComponent<T>();

                singleton.transform.SetParent(Constants.ControllerName);
            }
